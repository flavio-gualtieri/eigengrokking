...
from model.model import build_mlp, set_seed, build_toy_mpl
from analysis.eigenthings_new import estimate_density
from analysis.toy_eigenthings import compute_full_hessian, compute_hessian_eigenvalues, compute_hessian_spectrum_stats
        ...
        while steps < cfg.optimization_steps:
            ...
            if ((steps % cfg.spectral_every == 0) and (cfg.run_spectral == True) and (steps > 0)) or (steps == 1):
                try:
                    ...
                    eigvals = compute_hessian_eigenvalues(loss_spec, mlp)

                    eigvals_cpu = eigvals.detach().cpu()
                    step_key = steps
                    t_grid_store[step_key] = torch.arange(len(eigvals_cpu))
                    phi_store[step_key] = eigvals_cpu

                    print(
                        f"Step {steps}: "
                        f"min λ = {eigvals_cpu.min().item():.3e}, "
                        f"max λ = {eigvals_cpu.max().item():.3e}, "
                        f"trace = {eigvals_cpu.sum().item():.3e}, "
                        f"|λ|_max = {eigvals_cpu.abs().max().item():.3e}"
                    )

                    history["TOY_hessian_min_eig"].append(float(eigvals_cpu.min().item())) 
                    history["TOY_hessian_max_eig"].append(float(eigvals_cpu.max().item()))
                    history["TOY_hessian_trace"].append(float(eigvals_cpu.sum().item()))
                    history["TOY_hessian_spectral_radius"].append(float(eigvals_cpu.abs().max().item()))
                    history["TOY_hessian_steps"].append(steps)

                except Exception as e:
                    print(f"Warning: toy Hessian eigendecomposition failed at step {steps}: {e}")
