...
def _plot_training_with_right_axis(
    ...
    if len(train_log_steps) > 0:
        ax.plot(train_log_steps, train_accuracies, label="train", color="blue")
    if len(test_log_steps) > 0:
        ax.plot(test_log_steps, test_accuracies, label="test", color="red")
    ...
    ax2 = ax.twinx()
    if len(right_steps) > 0 and len(right_values) == len(right_steps):
        ax2.plot(
            right_steps,
            right_values,
            color="purple",
    ...
    dataset_info = f"MOD {cfg.modulus}" if cfg.dataset == "MODULAR" else "MNIST"
    train_info = (
        f"{cfg.train_points:.2f} frac"
        if isinstance(cfg.train_points, float)
        else f"{cfg.train_points} pts"
    )
    reg_info = f"WD={cfg.weight_decay}" if cfg.weight_decay > 0 else "No WD"

    ax.set_title(
        f"{dataset_info} | depth={cfg.depth}, width={cfg.width}, act={cfg.activation}\n"
        f"α={cfg.initialization_scale}, train={train_info}, "
        f"bs={cfg.batch_size}, steps={cfg.optimization_steps}, {reg_info}",
        fontsize=10,
    )
